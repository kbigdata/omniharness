...
🔒 강제 = 모델이 거부해도 코드가 막거나 실행한다. 🔁 자동 주입 = 모델이 요청하지 않아도 훅이 필요한 정보를 컨텍스트에 자동으로 넣어준다. ✍️ 권고 = 결국 모델이 해야 한다.
...
- **위험 명령 차단** — 모델이 `rm -rf /`·`sudo`·비밀키(`.env`·`id_rsa`)를 실수로 실행하려 하면 도구 실행 **직전**에 거부합니다. 권한 설정과 훅의 2겹 방어입니다.
- **완료 게이트** — 모델은 자기 결과를 "완료됐다"고 착각하기 쉽습니다(자기평가 편향). 그래서 **검증 없이는 완료가 불가능**합니다: `/omniharness:verify`가 독립 `evaluator`로 PASS/FAIL을 받아 기록하고, 그 PASS 기록이 없으면 **종료가 막힙니다**.
- **조기종료 차단** — `feature_list.json`에 미통과 기능이 남으면 종료를 막아, 작업을 마치지 않은 채 종료하는 것을 방지합니다(전제: 목록을 채워야 함).
- **작업 기록** — 모든 도구 호출을 `audit.jsonl`에 남김.

## 📚 위키 — 배운 것을 축적하고 자동으로 노출한다
...
- **플러그인 훅 자체는 무인 자율 루프가 아닙니다.** 훅은 세션 *안*에서 강제·검증만 합니다. 완전 자율 루프는 세션 *밖*의 드라이버 [`scripts/autoloop.py`](scripts/autoloop.py)가 담당합니다(**실험적** — 오프라인 48단언 + 실모델 검증: 단일 완주·위조 방어 2종·다기능 연쇄·실제 회귀 파손 감지·불가능 기능 보류 후 계속 진행). 설계·규율·검증 기록: [`docs/자동루프-설계.md`](docs/자동루프-설계.md).
...
- **위험 명령 차단은 최선노력 백스톱이지 샌드박스가 아닙니다.** 흔한 사고·직접적 시크릿 접근은 막지만, 난독화·base64·대체 도구 우회까지 막지는 못합니다. 진짜 경계는 Claude Code 권한 + `settings.json` + 신뢰 못 할 코드를 애초에 실행하지 않는 것입니다.
...
세션 *밖* 드라이버가 `feature_list.json`을 받아 **구현→모델 밖 검증→회귀→커밋**을 무인 반복합니다. 크로스플랫폼(Python+subprocess, SDK 없음).
...
# 야간 스케줄 무인 운영 — colima 기동→자격증명 주입→컨테이너 실행→로그→정리를 한 번에
bash scripts/nightly.sh          # OMNI_TARGET 으로 대상 프로젝트 지정(launchd/cron 에 등록해 사용)
```

> **주의**: 기본 `--permission-mode`는 `bypassPermissions`(evaluator가 테스트를 직접 실행하려면 필요) — **반드시 격리 환경(컨테이너)에서** 운영하세요. 무인 환경에선 `CLAUDE_BIN`으로 claude 절대경로 고정을 권장합니다. 검증: 오프라인 48단언 + 실모델(단일 완주·위조 방어·다기능 연쇄·회귀 파손 감지·막힘 보류/계속 진행) + **컨테이너 격리 무인 완주** + **야간 스케줄(launchd) 운영**(완료분은 재구현 없이 verify-first 재검증만). 5개 규율·차단점·운영 교훈: [`docs/자동루프-설계.md`](docs/자동루프-설계.md).

## 동작 확인

- **오프라인(API 키 불필요)**: `bash tests/run.sh` — 훅/게이트 스크립트에 샘플 입력을 넣어 48개 단언
...
- **라이브 통합(인증 필요)**: `bash tests/live.sh` — 실제 `claude --plugin-dir`로 훅 *배선*과 e2e 흐름을 검증
  ...
  - **SessionStart 주입(인계·위키 인덱스)은 라이브에선 best-effort(WARN)** — `-p` 단발 모드에서 `additionalContext`의 모델 도달이 불안정(실측 ~50%, 연속 6회 실행 0/6)하기 때문. **강제 단언은 오프라인**(`session_context.py` 직접 실행, 훅-출력 레벨)에서 결정적으로 한다.
